...
fit_file = paste("fitting_wt.stan",sep="")
...
for(i in 1:Nb){
  ...
  st_fit = sampling(stanmodel,
                    ...
                    iter   = iteration,
                    seed   = fitseed,
                    sample_file=paste("wt/sampling_",bat,"_","wt.csv",sep=""))
  ...
  #output
  write.csv(posterior_summary,paste("posterior_summary_",bat,"_",gen,".csv",sep=""),row.names=F)
}
...
for(i in 1:Nb){
  batch = batches[i]
  sampling_file_name = paste(gen,"/","sampling_",batch,"_",gen,sep="")
  chains = c(1,2,3,4)
